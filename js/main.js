...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyVTMlZ487Ti1UN1i8lMuO1WvUcPmL05yP4xKsGKKM4uEAPUJmVVqR7KOeKn0xw2N94/exec';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (claimsForm) {
    claimsForm.addEventListener('submit', submitClaimForm);
  }

  // ================= URL PARAMETERS PARSER (WhatsApp <-> Web Session Sync) =================
  const urlParams = new URLSearchParams(window.location.search);
  
  // 1. Detección de sesión de WhatsApp (?wsp=51999900396 o ?phone=...)
  ...
  if (wspParam) {
    ...
    // Personalizar botones "Volver a WhatsApp"
  ...
  // 2. Manejo de acción de anulación por URL (?action=cancelOrder&orderId=RP-2026-1042)
